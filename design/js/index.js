...
var weather = [
    { type: 'snow', name: 'Снег' },
    { type: 'wind', name: 'Ветер' },
    { type: 'rain', name: 'Дождь' },
    { type: 'thunder', name: 'Шторм' },
    { type: 'sun', name: 'Солнце' }
];
...
function onResize() {
    // рџ“Џ grab window and card sizes 
    ...
    TweenMax.fromTo(sunburst.node, 20, { rotation: 0 }, { rotation: 360, repeat: -1, ease: Power0.easeInOut })
        // рџЌѓ The leaf mask is for the leafs that float out of the
...
function drawCloud(cloud, i) {
    ...
    вЃпёЏ We want to create a shape thats loopable but that can also
...
function makeRain() {
    // рџ’§ This is where we draw one drop of rain
    ...
    // в›€ line length is made longer for stormy weather
...
function onRainEnd(line, width, x, type) {
    // first lets get rid of the drop of rain рџ’§
    ...
    if (rain.length < settings.rainCount) {
        ...
        // рџ’¦ If the line width was more than 2 we also create a 
...
function makeSplash(x, type) {
    // рџ’¦ The splash is a single line added to the outer svg.
...
function onSplashComplete(splash) {
    // рџ’¦ The splash has finished animating, we need to get rid of it
